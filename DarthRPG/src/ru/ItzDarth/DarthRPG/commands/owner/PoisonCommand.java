...
public class PoisonCommand extends RPGAbstractCommand {

    public PoisonCommand(String... commandNames) {
        super(commandNames);
    }
    ...
    public void executePlayer(Player p, PlayerDataRPG pd, String[] args) {
        ...
        if (target != null && target.isOnline() && plugin.getPD(target) != null) {
            plugin.getPD(target).givePoison(Integer.parseInt(args[1]), Integer.parseInt(args[2]));
            p.sendMessage("Gave " + target.getName() + " " + args[1] + " seconds of tier " + args[2] + " poison.");
        } else {
