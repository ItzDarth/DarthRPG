...
public abstract class SongPlayer {
    ...
    public void addPlayer(Player p) {
        synchronized (this) {
            if (!playerList.contains(p.getName())) {
                ...
                ArrayList<SongPlayer> songs = MusicManager.playingSongs.get(p.getName());
                if (songs == null) {
                    songs = new ArrayList<SongPlayer>();
                }
                songs.add(this);
                MusicManager.playingSongs.put(p.getName(), songs);
            }
        }
    }
    ...
    public void removePlayer(Player p) {
        synchronized (this) {
            ...
            if (MusicManager.playingSongs.get(p.getName()) == null) {
                return;
            }
            ArrayList<SongPlayer> songs = new ArrayList<SongPlayer>(MusicManager.playingSongs.get(p.getName()));
            songs.remove(this);
            MusicManager.playingSongs.put(p.getName(), songs);
